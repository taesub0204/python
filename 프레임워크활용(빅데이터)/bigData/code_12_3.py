...
ks = range(1, 10) # 군집 개수 후보
interias =  pd.Series([]) # 군집 평가 점수 저장할 빈 시리즈

for k in ks:
    model = KMeans(n_clusters = k, n_init = 10, random_state = 123) # 군집 개수 k로 설정
    model.fit(df_scaled)
    interias[k] = model.inertia_ # 군집 평가 점수 저장

plt.figure(figsize = (7, 4))
interias.plot.line(title = 'Inertias Score', xlabel = 'number of clusters, k', ylabel = 'inertia')
plt.show()