...
def radar(df, fills, min_max, title=""):
    ...
    # 레이아웃 설정은 그래프를 다 그린 후 '루프 밖에서 한 번만' 실행합니다.
    fig.update_layout(
    ...
    # 복잡한 이미지 저장 및 matplotlib 코드를 지우고, 내장 기능으로 시각화합니다.
    fig.show()
...
# 2. 전체 학생 점수 그래프 호출
fills_students = ["toself", "toself", "toself", "toself"]
radar(
    df=df.iloc[1:, :],
    fills=fills_students,
    min_max=[0, 100],
