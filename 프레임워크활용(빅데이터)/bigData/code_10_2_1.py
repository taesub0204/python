...
df = pd.read_fwf('C:/Users/user/Desktop/taesub/python/프레임워크활용(빅데이터)/data/반기문_국회_연설문.txt').iloc[:,0]
df.head()
...
# 불필요한 단어 제거
del_idx = df_word.loc[df_word.word.isin(['우리','한국','여러분','국민','사회'])].index
...
import numpy as np
mask = np.array(Image.open('C:/Users/user/Desktop/taesub/python/프레임워크활용(빅데이터)/data/circle.png'))
...
wc = WordCloud(random_state = 123,
               font_path = 'C:/Windows/Fonts/HMFMPYUN.TTF',
                ...
                contour_color = 'steelblue', # 등고선 색상
                contour_width = 2 # 등고선 두께
                )
img_wordcloud = wc.generate_from_frequencies(dic_word) #워드 클라우드 > 이미지
