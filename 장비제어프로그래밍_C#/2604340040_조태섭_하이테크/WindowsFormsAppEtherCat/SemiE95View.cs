...
namespace WindowsFormsAppEtherCat
{
    public partial class SemiE95View : UserControl
    {
        ...
        // INIT 버튼 (구 LOAD): 장비 초기화를 한 번에 수행합니다.
        // 서보 ON → 드라이브 준비 대기 → 파라미터 적용 → Z/X 원점복귀 → 원점(0 부근) 정렬.
        // [개선 배경] 기존에는 리셋 후 X/Z축이 초기값으로 돌아오지 않아 수동으로 맞춰야 로직이
        // 동작하는 문제가 있었음. 원인은 서보 ON 직후 대기 없이 명령을 보내 드라이브가 준비 전에
        // 호밍/파라미터를 흘려버리는 순서 문제 + 호밍 안착 위치가 0이 아니라 1500 안팎인 장비 특성.
        // → ON 후 1500ms 대기, 호밍 완료 확인 후 0점으로 미세 정렬 이동까지 수행하도록 개선.
        // (RESET 버튼은 제거되었고, 그 역할을 이 버튼이 흡수함)
        private async void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                // [진단용] 동작 중 STOP → INIT 시나리오에서 원점 정렬이 실패하는 원인을 추적하기 위해,
                // 초기화 시작 시점의 실제 상태(직전 위치, 블레이드 센서)를 로그에 남깁니다.
                string bladeStateAtStart = "(읽기실패)";
                try { bladeStateAtStart = EtherCAT_M.Digital_Input(12).ToString(); } catch { }
                LoggerConfig.Log.Info($"[INIT추적] 시작 — 이전 X={EtherCAT_M.Axis2_is_PosData()}, 이전 Z={EtherCAT_M.Axis1_is_PosData()}, 블레이드후진센서(DI12)={bladeStateAtStart}");

                // 1. 서보 모터 전원 ON
                EtherCAT_M.Axis1_ON();
                EtherCAT_M.Axis2_ON();

                // 2. [중요] 드라이브가 OP 상태로 완전히 전환될 때까지 대기.
                //    대기 없이 Config_Update/호밍을 보내면 드라이브가 준비 전이라 명령을 흘려버려
                //    축이 초기값으로 돌아오지 않는("드라이브 무응답") 현상의 원인이 됨.
                ...
                // 3.5 [실측 기반 개선] 호밍 전에 먼저 0 근처로 직접 이동시켜 둡니다.
                // 로그 실측 결과, 원점에서 아주 멀리 떨어진 상태(예: STOP 직후 챔버 근처, Z=-65775)에서
                // Axis1_UD_Homming()을 걸면 초당 약 20count 수준의 저속 이동으로 동작해 40초 타임아웃
                // 안에는 어림도 없이 못 끝나고 사실상 멈춘 것처럼 보임(실측: 40초에 792count만 이동).
                // 반면 직접 POS_Update+Move_Send는 같은 거리를 1초 이내에 주파함(실측: -66330→0, 600ms).
                // 그래서 호밍 전에 먼저 직접 이동으로 0 근처까지 데려다 놓아서, 실제 호밍은 원점 스위치를
                // 찾는 짧은 거리 정밀 보정만 하도록 만든다(X축은 이미 원점 근처였을 때 호밍이 2초 만에
                // 끝난 것과 동일한 조건을 Z축에도 만들어주는 것).
                try
                {
                    EtherCAT_M.Axis1_UD_POS_Update(0);
                    EtherCAT_M.Axis1_UD_Move_Send();
                    await WaitForAxis1Async(0, CancellationToken.None, 5000);

                    EtherCAT_M.Axis2_LR_POS_Update(0);
                    EtherCAT_M.Axis2_LR_Move_Send();
                    await WaitForAxis2Async(0, CancellationToken.None, 5000);

                    LoggerConfig.Log.Info($"[INIT추적] 호밍 전 사전 이동 완료 — X={EtherCAT_M.Axis2_is_PosData()}, Z={EtherCAT_M.Axis1_is_PosData()}");
                }
                catch (Exception preEx)
                {
                    LoggerConfig.Log.Warn($"[INIT추적] 호밍 전 사전 이동 실패(호밍 단계로 계속 진행): {preEx.Message}");
                }

                // 4. Z축(상하) 원점복귀 → 완료 대기 (원점 센서 안착 위치가 0이 아니라 1500 안팎이라 오차 3000)
                //    이제 사전 이동으로 0 근처에서 시작하므로, 호밍은 짧은 거리 정밀 보정만 하면 됨.
                LoggerConfig.Log.Info($"[INIT추적] Z축 호밍 시작 — 현재 Z={EtherCAT_M.Axis1_is_PosData()}");
                EtherCAT_M.Axis1_UD_Homming();
                await WaitForAxis1Async(0, CancellationToken.None, 3000, true);
                await Task.Delay(1000); // 호밍 완료 후 드라이브 안정화 대기
                LoggerConfig.Log.Info($"[INIT추적] Z축 호밍 완료 — 결과 Z={EtherCAT_M.Axis1_is_PosData()}");

                // 5. X축(좌우) 원점복귀 → 완료 대기 (Z축 원점복귀 완료 후에만 가능하다는 장비 규칙)
                LoggerConfig.Log.Info($"[INIT추적] X축 호밍 시작 — 현재 X={EtherCAT_M.Axis2_is_PosData()}");
                EtherCAT_M.Axis2_LR_Homming();
                await WaitForAxis2Async(0, CancellationToken.None, 3000, true);
                await Task.Delay(1000);
                LoggerConfig.Log.Info($"[INIT추적] X축 호밍 완료 — 결과 X={EtherCAT_M.Axis2_is_PosData()}");

                // 6. 호밍이 드라이브 이송 프로파일을 리셋하므로 파라미터 재적용
                EtherCAT_M.Axis1_UD_Config_Update(defaultAccel, defaultDecel, defaultMaxSpeed, defaultSpeed);
                EtherCAT_M.Axis2_LR_Config_Update(defaultAccel, defaultDecel, defaultMaxSpeed, defaultSpeed);

                // [중요] Config_Update 직후 곧바로 이동 명령을 보내면 드라이브가 아직 파라미터를
                // 완전히 반영하기 전이라 명령이 무시될 수 있음(이 파일 전반에 걸쳐 반복 확인된 패턴).
                // 수동 조작(Form1)에서는 사용자가 파라미터 적용 버튼(button39)을 누르고 팝업을 확인한
                // 뒤 몇 초 지나서야 이동 버튼을 누르기 때문에 자연스럽게 이 대기 시간이 생겨서 잘 됐던 것.
                // 자동화된 INIT에서는 그 텀이 없었으므로 동일한 효과를 내도록 명시적으로 대기를 넣는다.
                await Task.Delay(300);

                // 7. 원점(0 부근)으로 미세 정렬 이동 — 호밍 안착 위치(~1500)를 실제 초기값(0)으로 맞춤.
                //    [개선] WaitForAxis1/2Async는 40초 타임아웃에 도달해도 예외 없이 조용히 리턴할 수 있으므로
                //    (해당 타임아웃 알람은 현재 비활성화 상태), 이동 명령을 보냈다는 것만으로 실제로 0 근처에
                //    도달했다고 보장할 수 없음. 이동 후 실제 값을 직접 읽어 0과의 편차를 확인하고,
                //    벗어나 있으면 최대 3회까지 재시도로 보정한다.
                const long zeroTolerance = 500; // 0 부근으로 인정하는 허용오차(count)
                const int maxAlignAttempts = 3;
                bool aligned = false;
                long finalX = 0, finalZ = 0;

                for (int attempt = 1; attempt <= maxAlignAttempts && !aligned; attempt++)
                {
                    try
                    {
                        EtherCAT_M.Axis1_UD_POS_Update(0);
                        EtherCAT_M.Axis1_UD_Move_Send();
                        await WaitForAxis1Async(0, CancellationToken.None, zeroTolerance);

                        EtherCAT_M.Axis2_LR_POS_Update(0);
                        EtherCAT_M.Axis2_LR_Move_Send();
                        await WaitForAxis2Async(0, CancellationToken.None, zeroTolerance);
                    }
                    catch (Exception moveEx)
                    {
                        // 이동 중 "드라이브 무응답"이 떠도 마지막 시도가 아니면 바로 알람으로 죽이지 않고
                        // 재시도한다(파라미터 반영 지연 등 일시적 원인일 수 있음).
                        LoggerConfig.Log.Warn($"INIT 원점 정렬 {attempt}차 시도 중 이동 오류(재시도 예정): {moveEx.Message}");
                    }

                    finalX = ParseAxisPos(EtherCAT_M.Axis2_is_PosData());
                    finalZ = ParseAxisPos(EtherCAT_M.Axis1_is_PosData());
                    aligned = Math.Abs(finalX) <= zeroTolerance && Math.Abs(finalZ) <= zeroTolerance;

                    if (!aligned)
                    {
                        LoggerConfig.Log.Warn($"INIT 원점 정렬 {attempt}차 시도: X={finalX}, Z={finalZ} — (0,0)과 편차가 커서 재보정 시도");
                        await Task.Delay(300); // 재시도 전 짧은 안정화 대기
                    }
                }

                if (!aligned)
                {
                    throw new Exception($"원점 정렬 실패: {maxAlignAttempts}회 보정 시도 후에도 (0,0)과 편차가 큽니다. (X={finalX}, Z={finalZ})");
                }

                LoggerConfig.Log.Info($"INIT 완료: X={finalX}, Z={finalZ} (0,0 근사 확인됨)");
                ChangeState(SystemState.READY); // 장비 준비 완료!
            }
            catch (Exception ex)
            {
                TriggerAlarm("INIT-ERR", "장비 초기화 실패: " + ex.Message);
            }
            finally
            {
        ...
        // (RESET 버튼은 제거됨 — 서보 재기동/원점복귀는 INIT 버튼이 수행합니다)

        // ALARM RESET: 서보 재기동/재원점복귀 없이, 알람 상태만 가볍게 해제합니다.
        // (재원점복귀까지 필요하면 INIT 버튼을 사용)
        private void btnAlarmReset_Click(object sender, EventArgs e)
        {
            ...
            UpdateFoupBStatus();
            ChangeState(SystemState.INIT);
            // 알람 해제 시 타워램프(적색)도 함께 소등 (INIT 버튼과 동일한 동작)
            try { SetTowerLamp(false, false, false); } catch { }
        }
        ...
        private async Task RobotPickAsync(string source, int slot, CancellationToken token)
        {
            ...
            // [인터록 ①] 블레이드 전진 상태에서는 X축 좌우 이동 금지
            VerifyBladeRetractedBeforeMove("X축");
            ...
            // 블레이드가 챔버 안으로 들어가려면 반드시 도어가 열려있어야 하므로
            ...
            // 3. Z축 안착 위치(Lower)로 이동 및 도착 대기
            // [인터록 ②] 블레이드 전진 상태에서는 Z축 상하 이동 금지
            // (이후 4~7단계의 상승/하강은 블레이드로 웨이퍼를 들고 내리는 Pick 동작 자체이므로 예외)
            VerifyBladeRetractedBeforeMove("Z축");
            await MoveAxis1SafeAsync(placeZ, token);

            // [인터록 ③] 챔버일 경우, 도어가 열린 상태에서만 블레이드 전진 허용 (센서 재확인)
            if (isChamber)
            {
                VerifyDoorOpenBeforeBlade(source);
            }

            // 4. 블레이드 전진 (센서 확인) — 위 검증을 통과한 후에만 실행
            ...
            // [인터록 ④] 챔버일 경우, 도어가 열린 상태에서만 블레이드 후진 허용 (센서 재확인)
            if (isChamber)
            {
                VerifyDoorOpenBeforeBlade(source);
            }
            ...
            // [동작 개선] 기존에는 여기서 Z축을 안착 위치(Lower)로 복귀시킨 뒤 X축 이동을 했으나,
            // 안착 위치를 불필요하게 여러 번 거치는 동작이라 제거함.
            // 이제 "블레이드 후진 → X축 이동 → 목적지에서 상황에 맞게 Z축 이동" 순서로 동작합니다.
        }
        ...
        private async Task RobotPlaceAsync(string dest, int slot, CancellationToken token)
        {
            ...
            // [인터록 ②] 블레이드 전진 상태에서는 Z축 상하 이동 금지
            // (이후 4~7단계의 하강은 블레이드로 웨이퍼를 내려놓는 Place 동작 자체이므로 예외)
            VerifyBladeRetractedBeforeMove("Z축");
            await MoveAxis1SafeAsync(pickZ, token);
        ...
        // -------------------------------------------------------------
        // [핵심 인터록 4가지] — 과도한 인터록을 정리하고 아래 4가지만 유지
        //   ① 블레이드 전진 상태에서 X축 좌우 이동 금지  → VerifyBladeRetractedBeforeMove
        //   ② 블레이드 전진 상태에서 Z축 상하 이동 금지  → VerifyBladeRetractedBeforeMove
        //   ③ 챔버 도어가 열린 상태에서만 블레이드 전진  → VerifyDoorOpenBeforeBlade
        //   ④ 챔버 도어가 열린 상태에서만 블레이드 후진  → VerifyDoorOpenBeforeBlade
        // -------------------------------------------------------------

        // [인터록 ①②] 축 이동 직전, 블레이드가 후진(DI12=1) 상태인지 확인합니다.
        // 블레이드가 전진(챔버/FOUP 안에 진입)된 상태에서 축이 움직이면 충돌하므로 이동을 차단합니다.
        private void VerifyBladeRetractedBeforeMove(string axisName)
        {
            string bladeRetractState = EtherCAT_M.Digital_Input(12).ToString().ToLower();
            if (bladeRetractState != "1" && bladeRetractState != "true")
            {
                throw new Exception($"[인터록 알람] 블레이드가 전진 상태입니다! 충돌 방지를 위해 {axisName} 이동을 차단합니다. (DI12 원시값: {bladeRetractState})");
            }
        }

        // [인터록 ③④] 블레이드 전진/후진 직전, 챔버 도어가 실제로(센서 기준) 열려있는지 확인하는 함수
...

        {
            ...
            // (이동 시간 초과 알람은 현재 비활성화 상태 — 대신 위 무응답 조기 감지가 대신함)
        ...
        // 축 위치 문자열(예: "12345 count")에서 정수값만 추출합니다. 파싱 실패 시 0을 반환합니다.
        private long ParseAxisPos(string raw)
        {
            string numStr = System.Text.RegularExpressions.Regex.Match(raw ?? "", @"-?\d+").Value;
            return long.TryParse(numStr, out long pos) ? pos : 0;
        }
