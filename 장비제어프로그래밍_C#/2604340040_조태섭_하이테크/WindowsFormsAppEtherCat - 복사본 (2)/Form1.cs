...
namespace WindowsFormsAppEtherCat
{
    public partial class Form1 : Form
    {
        ...
        // private Panel panelHome; 대신에 아래처럼 HomeView로 바꿉니다.
        private HomeView panelHome;
        ...
        private void button40_Click(object sender, EventArgs e)
        {
            ...
            if (label10.Text == "false")
            {
                EtherCAT_M.Axis2_LR_POS_Update((Int64)numericUpDown6.Value);
                EtherCAT_M.Axis2_LR_Move_Send();
                // numericUpDown6 값만 큼 이동 합니다.
                MessageBox.Show("좌우 이동 명령 전송됨!\n목표 값: " + numericUpDown6.Value);
            }
            else
            { 
                MessageBox.Show("블레이드 전진 상태에서는 좌우 이동이 불가합니다. 블레이드를 후퇴시킨 후 다시 시도해주세요.");
        ...
        private void button42_Click(object sender, EventArgs e)
        {
            ...
            if (label10.Text == "false")
            {
                EtherCAT_M.Axis1_UD_POS_Update((Int64)numericUpDown6.Value);
                EtherCAT_M.Axis1_UD_Move_Send();
                // numericUpDown6 값만 큼 이동 합니다.
                MessageBox.Show("상하 이동 명령 전송됨!\n목표 값: " + numericUpDown6.Value);
            }
        ...
        {
            // [수정] 일반 패널 대신에 디자인창으로 꾸민 HomeView를 생성합니다.
            panelHome = new HomeView();
            panelHome.Dock = DockStyle.Fill;
            panelHome.Visible = false;

            // 이제 코드 상에서 라벨을 일일이 만들 필요가 없습니다! (디자이너에서 다 하니까요)
            // 아래 폼 추가 코드는 그대로 유지합니다.
